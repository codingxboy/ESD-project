...
import com.yunhe.common.core.filter.StrAttackFilter;
...
public class OssBootUtil {
    ...
    public static String upload(MultipartFile file, String fileDir,String customBucket) {
        ...
        if(oConvertUtil.isNotEmpty(customBucket)){
        ...
        try {
            ...
            if (!fileDir.endsWith(SymbolConstants.SINGLE_SLASH)) {
                fileDir = fileDir.concat(SymbolConstants.SINGLE_SLASH);
            }
            ...
            if (oConvertUtil.isNotEmpty(staticDomain) && staticDomain.toLowerCase().startsWith(CommonConstants.STR_HTTP)) {
                filePath = staticDomain + SymbolConstants.SINGLE_SLASH + fileUrl;
            } else {
                filePath = "https://" + newBucket + "." + endPoint + SymbolConstants.SINGLE_SLASH + fileUrl;
            }
...
//            ossClient.setBucketAcl(newBucket, CannedAccessControlList.PublicRead);
    ...
    */
    public static String getOriginalUrl(String url) {
        ...
        if(oConvertUtil.isNotEmpty(staticDomain) && url.indexOf(staticDomain)!=-1){
    ...
    public static String upload(FileItemStream file, String fileDir) {
        ...
        try {
            ...
            if (oConvertUtil.isNotEmpty(staticDomain) && staticDomain.toLowerCase().startsWith(CommonConstants.STR_HTTP)) {
                ...
                filePath = "https://" + bucketName + "." + endPoint + SymbolConstants.SINGLE_SLASH + fileUrl;
            }
    ...
    public static void deleteUrl(String url,String bucket) {
        String newBucket = bucketName;
        if(oConvertUtil.isNotEmpty(bucket)){
            newBucket = bucket;
        }
        String bucketUrl = "";
        if (oConvertUtil.isNotEmpty(staticDomain) && staticDomain.toLowerCase().startsWith(CommonConstants.STR_HTTP)) {
            bucketUrl = staticDomain + SymbolConstants.SINGLE_SLASH ;
        } else {
            bucketUrl = "https://" + newBucket + "." + endPoint + SymbolConstants.SINGLE_SLASH;
        }
    ...
    public static String upload(InputStream stream, String relativePath) {
        ...
        initOss(endPoint, accessKeyId, accessKeySecret);
        if (oConvertUtil.isNotEmpty(staticDomain) && staticDomain.toLowerCase().startsWith(CommonConstants.STR_HTTP)) {
            filePath = staticDomain + SymbolConstants.SINGLE_SLASH + relativePath;
        } else {
            filePath = "https://" + bucketName + "." + endPoint + SymbolConstants.SINGLE_SLASH + fileUrl;
        }
    ...
    private static String replacePrefix(String objectName,String customBucket){
        ...
        if(oConvertUtil.isNotEmpty(staticDomain)){
            objectName= objectName.replace(staticDomain+SymbolConstants.SINGLE_SLASH,"");
        }else{
            String newBucket = bucketName;
            if(oConvertUtil.isNotEmpty(customBucket)){
                newBucket = customBucket;
            }
            String path ="https://" + newBucket + "." + endPoint + SymbolConstants.SINGLE_SLASH;
