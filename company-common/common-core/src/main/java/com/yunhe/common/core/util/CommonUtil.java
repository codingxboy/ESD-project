...
import com.yunhe.common.core.constants.CommonConstants;
import com.yunhe.common.core.constants.DataBaseConstants;
import com.yunhe.common.core.constants.ServiceNameConstants;
import com.yunhe.common.core.constants.SymbolConstants;
import com.yunhe.common.core.filter.FileTypeFilter;
...
//        fileName += "." + PoiPublicUtil.getFileExtendName(data);
        try {
            if(CommonConstants.UPLOAD_TYPE_LOCAL.equals(uploadType)){
                ...
                if(CommonConstants.UPLOAD_TYPE_MINIO.equals(uploadType)){
                    dbPath = MinioUtil.upload(in,relativePath);
                }else if(CommonConstants.UPLOAD_TYPE_OSS.equals(uploadType)){
    ...
    public static String uploadLocal(MultipartFile mf,String bizPath,String uploadpath){
        try {
            ...
            if(orgName.indexOf(SymbolConstants.SPOT)!=-1){
            ...
            String dbpath = null;
            if(oConvertUtil.isNotEmpty(bizPath)){
                dbpath = bizPath + File.separator + fileName;
            }else{
                dbpath = fileName;
            }
            if (dbpath.contains(SymbolConstants.DOUBLE_BACKSLASH)) {
    ...
    public static DbType getDatabaseTypeEnum() {
        if (oConvertUtil.isNotEmpty(dbTypeEnum)) {
            return dbTypeEnum;
        }
        try {
            DataSource dataSource = SpringContextUtil.getApplicationContext().getBean(DataSource.class);
    ...
    private static String getDatabaseTypeByDataSource(DataSource dataSource) throws SQLException{
        if("".equals(DB_TYPE)) {
            ...
            try {
                ...
                if(dbType.indexOf(DataBaseConstants.DB_TYPE_MYSQL)>=0) {
                    DB_TYPE = DataBaseConstants.DB_TYPE_MYSQL;
                }else if(dbType.indexOf(DataBaseConstants.DB_TYPE_ORACLE)>=0 ||dbType.indexOf(DataBaseConstants.DB_TYPE_DM)>=0) {
                    DB_TYPE = DataBaseConstants.DB_TYPE_ORACLE;
                }else if(dbType.indexOf(DataBaseConstants.DB_TYPE_SQLSERVER)>=0||dbType.indexOf(sqlserver)>=0) {
                    DB_TYPE = DataBaseConstants.DB_TYPE_SQLSERVER;
                }else if(dbType.indexOf(DataBaseConstants.DB_TYPE_POSTGRESQL)>=0) {
                    DB_TYPE = DataBaseConstants.DB_TYPE_POSTGRESQL;
                }else if(dbType.indexOf(DataBaseConstants.DB_TYPE_MARIADB)>=0) {
                    DB_TYPE = DataBaseConstants.DB_TYPE_MARIADB;
                }else {
    ...
    public static String getBaseUrl(HttpServletRequest request) {
        ...
        if(oConvertUtil.isNotEmpty(xGatewayBasePath)){
        ...
        String scheme = request.getHeader(CommonConstants.X_FORWARDED_SCHEME);
        if(oConvertUtil.isEmpty(scheme)){
